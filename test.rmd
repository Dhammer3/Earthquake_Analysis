...
##SHINY FRONT END
ui <- fluidPage(
  titlePanel("Earthquakes by Intensity and Location"),
  sidebarLayout(
    sidebarPanel(
      
      
      sliderInput("magInput", "Magnitude", 5, 10, c(5, 7))
    ),
    mainPanel(
...
server <- function(input, output) {
  output$mymap <- renderLeaflet({
    ...
    filtered <- get_quakes_by_mag_window(input$magInput[1],input$magInput[2])
          leaflet(data = filtered[1:max_num_points,]) %>% addTiles() %>%
    addMarkers(~Longitude, ~Latitude, popup = ~as.character(Magnitude), label = ~as.character(Date))
